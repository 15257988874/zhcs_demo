                ...
                if (this.cfg.click) {
                    if (_this.cfg.treeCfg) {
                        $('#' + this.id[0]).click(function () {
                            _this.cfg.click($.extend(_this.cfg.treeCfg, {
                                elm: $(this)
                            }));
                        });
                    } else {
                        $('#' + this.id[0]).click(function () {
                            ...
                            _this.cfg.click($.extend(_this.cfg.cfg, {
                                elm: $(this)
                            }), res);
                        });
                    }
    ...
    AQInput.prototype = {
        _init: function (vType) {
            ...
            if (dt == 'text' || dt == 'date') {
                ...
                arr = shortc.values.split('|');
                var text = shortc.text.split('|');
            ...
            if (this.cfg.dt == 'date') {
                for (var i = 0; i < this.id.length; i++) {
                    ...
                    if (this.cfg.cfg) {
                        this.dateCfg.push(this.cfg.cfg);
                    }
                }
            }
            return this;
    ...
    Autoform.prototype = {
        init: function (cfg) {
            ...
            if (!cfg.isAdvanced && cfg.pcfg) {
                ...
                var btnShow = cfg.pcfg.btns !== undefined ? cfg.pcfg.btns : true,
                    picbtn = $('<li/>', {
                        class: 'block-item layui-form-item'
                    }),
                    lab = $('<label/>', {
                        text: '图片信息:'
                    }),
                    ...
                    picwrap = $('<div/>', {
                        class: "yy-m-form-picview-wrap"
                    }),
                    picbox = $('<div/>', {
                        class: "yy-m-form-picview"
                    }),
                    btn1 = $('<a/>', {
                        class: cfg.pcfg.auto ? 'layui-btn  layui-btn-sm' : 'layui-btn layui-btn-normal  layui-btn-sm',
                        id: 'ycya-up-btn1',
                        text: cfg.pcfg.auto ? '图片上传' : '选择图片'
                    });
                picwrap.css({
                    'marginTop': '-32px'
                }).append(picbox);
            ...
            if (cfg.map && cfg.map.btns) {
                ...
                var mapLi = $('<li/>', {
                    ...
                    mapDiv = $('<div/>', {
                        class: 'm-inline-block w-all',
                        id: this.popupPara.btnToAdmin
                    }),
                    mapBtn1 = $('<button/>', {
            ...
            if (!cfg.isAdvanced && cfg.pcfg) {
                this.preview = $('.yy-m-form-picview', '#' + cfg.id);
            }
            //图片按钮事件
            if (!cfg.isAdvanced && cfg.pcfg) {
                //图片按钮绑定事件
                    ...
                    choose: function (obj) {
                        ...
                        if (!cfg.pcfg.multiple) {
                            ...
                            if (cfg.pcfg.max && len > cfg.pcfg.max) {
                                layer.alert('不能超过' + cfg.pcfg.max + '张图片', {
                                    icon: 5
                                });
            ...
            if (this.selectList.length > 0) { //初始化select
                ...
                $.each(this.selectList, function (i, item) {
                    if (item.cfg && item.cfg.url) {
                        var cloneCfg = $.extend(true, {}, app.ui.formSelectsCfg, {
                            type: item.cfg.type || 'post',
                            searchUrl: item.cfg.url
                        });
                        ...
                        if (_this.pwd.length > 0) { //密码框MD5传输
                            for (var p = 0; p < _this.pwd.length; p++) {
                                var item = _this.pwd[p],
                                    v=$('#' + cfg.id + ' [name="' + item + '"]').val();
                                if(v.length<32){//已经加密
                                    $.trim(v) !== '' &&  (data.field[item] = hex_md5(v));
                                }else{
                                    data.field[item]=v;
                                }
                            }
                    ...
                    (function (k) {
                        if (_this.eventName[k] === "tree") {
                            ...
                            $('#' + cfg.id).on('click', '[name="' + _this.bindElm[k] + '"]', function () {
                                _this.tree($.extend(_this.cfgFn[k], {
                                    elm: $(this)
                                }));
                            });
                            ...
                            $('#' + cfg.id).on('click', '[name="' + _this.bindElm[k] + '"]', function () {
                                _this.getPlace($.extend(_this.cfgFn[k], {
                                    elm: $(this)
                                }));
                            });
                        } else {
        ...
        set: function (valueJson, ids, addrNames, picName, extra) {
                ...
                $.each(extra, function (i, item) {
                    if (i == 'selLinkage') { //联动select数据回显
                        for (var j = 0; j < item.length; j++) {
                            ...
                            var cloneCfg = $.extend(true, {}, app.ui.formSelectsCfg, {
                                    type: 'post',
                                    searchUrl: !$.isEmptyObject(testCfg) && testCfg.url
                                }),
            ...
            if (addrNames && $.type(addrNames) === 'array' && addrNames.length == 2) {
                for (var o = 0; o< this.address.length; o++) {
                    $('#' + this._cfg.id + ' [name=' + this.address[o] + ']').attr('data-lng', valueJson[addrNames[0]]).attr('data-lat', valueJson[addrNames[1]]);
                }
            }
            //图片
            if (picName) {
                ...
                if (this.pic && this.preview && (valueJson[picName] != '' && valueJson[picName] != '0')) {
                    if (valueJson[picName].indexOf(',') === -1) {
                        ...
                        pimg = [];
                        this.files = {};
                        for (var n = 0, l = plist.length; n < l; n++) {
                            pimg.push('<span><img src="' + app.url + 'file/view?id=' + plist[n] + '" class="layui-upload-img"><i class="layui-icon mod-img" data-id=' + plist[n] + '>&#x1006;</i></span>');
                            this.pic[n] = plist[n];
                            this.files[n] = plist[n];
                        }
                    }
        ...
        createAdmin: function () {
            ...
            if (!ztreeflag) {
                ...
                _jsLoader.loadFileList([getContentPath() + '/res/dep/zTree/css/zTreeStyle/zTreeStyle.css',
                    getContentPath() + '/res/dep/zTree/js/jquery.ztree.all.js'
                ], function () {});
            }
        },
        openAdmin: function () {
                            ...
                            onClick: function (event, treeId, NowtreeNode) {
                                ...
                                if (NowtreeNode.level == 1) {
                                    var pNode = NowtreeNode.getParentNode(),
                                        newNode, sNode;
                                    for (var k in d) {
                                        if (d[k].adcode == pNode.adcode) {
                                            newNode = d[k].districts;
...
app.ui.util = {
    cssMerge: function (cssCfg, defaultCss) { //css合并
        if (cssCfg.css) {
            ...
            for (var idx2 = delArr.length - 1; idx2 >= 0; idx2--) {
                cssCfg.css.splice(idx2, 1);
            }
...
app.ui.form = {
    ...
    init: function (cfg) {
        ...
        var h = cfg.data.items.length * 38 + 100,
            wh = $(window).height() - 40;
        h = h > wh ? wh : h;
        var _this = this;
        layer.open($.extend({
...
app.ui.search = {
    ...
    init: function (cfg, extraPra) {
        ...
        $(this._cfg.t, dom).html(this._cfg.title);
        ...
        $(window).resize(function () {
            var cw = $(_this._cfg.s).width() + $(_this._cfg.t).width(),
                pw = $(_this._cfg.s).parent().width(),
                cItem = $('.yy-m-search-item', _this._cfg.s);
            if (initW >= window.innerWidth || initH >= window.innerHeight) { //缩小
                if (cw >= pw) { //溢出
                    for (var i = cItem.length - 1; i > 0; i--) {
                        $(cItem[i]).hide();
                        if ($(_this._cfg.s).width() + $(_this._cfg.t).width() < $(_this._cfg.s).parent().width()) {
                            break;
                        }
                    }
                }
            } else {
                for (var k = cItem.length - 1; k > 0; k--) {
                    $(cItem[k]).show();
                    if ($(_this._cfg.s).width() + $(_this._cfg.t).width() > $(_this._cfg.s).parent().width()) {
                        $(cItem[k]).hide();
                        break;
...
app.ui.page.list = {
    init: function (cfg) {
        ...
        if (cfg.left) this.left = app.ui.left.init($.extend(cfg.left, {
            id: 'yui-left'
        }));
        if (cfg.search) this.search = app.ui.search.init($.extend(cfg.search, {
            id: 'yui-search'
        }));
        if (cfg.grid) this.grid = app.ui.grid.init($.extend(cfg.grid, {
            elem: '#yui-grid'
        }));
        return this;
    },
...
app.ui.page.bindpopup = {
    init: function (cfg) {
        this.cfg = cfg;
        if (cfg.search) {
            this.search = app.ui.search.init(cfg.search, {
                dom: $('#pop-search')
            });
        }
        if (cfg.selected) {
            this.selected = app.ui.selected.init(cfg.selected);
        }
        if (cfg.grid) {
            this.tableGrid = app.ui.grid.init($.extend(cfg.grid, {
                height: cfg.selected ? 225 : 288,
                elem: '#pop-grid'
            }));
        }
        return this;
...
app.ui.reportSearch = {
    ...
    init: function (cfg) {
        ...
        if (c.yearQuery) { //年查询
            ...
            if (c.yearQueryBack) {
                for (var j = c.yearNum - 1; j >= 0; j--) {
                    _class = j === 0 ? ['yy-lf', 'active'] : ['yy-lf'];
                    _yearQuery.append($('<li class="' + _class.join(' ') + '">' + (_nowYear - j) + '</li>'));
                }
            } else {
                for (var s = 0; s < c.yearNum; s++) {
                    _class = s === 0 ? ['yy-lf', 'active'] : ['yy-lf'];
                    _yearQuery.append($('<li class="' + _class.join(' ') + '">' + (_nowYear + s) + '</li>'));
                }
            }
        ...
        if (c.monthQuery) { //月查询
            ...
            for (var t = 1; t < 13; t++) {
                _class = t === _nowMonth ? ['yy-lf', 'active'] : ['yy-lf'];
                _monthQuery.append($('<li class="' + _class.join(' ') + '">' + t + '</li>'));
            }
    ...
    formInit: function () {
        ...
        this.fields = fields;
        $('.yy-m-search', _dom).append($('<button/>', {
            class: 'layui-btn layui-btn-sm layui-btn-normal',
            text: '确定',
            click: function () {
                //时间判断
...
app.ui.report = {
    init: function (cfg) {
        if (cfg.title) {
            this.title = app.ui.reportTitle.init(cfg.title);
        }
        if (cfg.search) {
            this.search = app.ui.reportSearch.init(cfg.search);
        }
        if (cfg.echart) {
            if (!cfg.echart.type) {
                cfg.echart.type = 'bar';
    ...
    init: function (cfg) {
        ...
        $.each(_c.item, function (i, item) {
            ...
            _slist.append($('<li/>', {
                ...
                width: 1 / _c.item.length * 100 + '%',
                html: '<p class="' + cl + '" data-status=' + item.status + '>' + item.num + '</p><p>' + item.text + '</p>'
            })[0].outerHTML);
        });
        ...
        if (_c.toggle) {
            $('#mapWrap').append($('<div/>', {
                class: 'run-status-box-toggle cursor-point',
                text: '运行情况>>',
                click: function () {
                    $('#' + boxId).toggle(400);
    ...
    init: function (cfg) {
        ...
        $.each(cfg, function (i, item) {
            var cl = ['iconfont', 'm-right-4', item.class].join(' '),
                ...
                _li = $('<li/>', { /* click:item.handle, */
                    id: item.id,
                    html: '<i class="' + cl + '"></i><span>' + item.name + '</span><i class="' + cl2 + '"></i>',
                    class: item.cursor ? 'cursor-point' : ''
                });
            if (item.event) {
                var iobj = {
                    id: item.id,
        ...
        //插入退出全屏
        this.exit = '#yyExit';
        $('#mapWrap').length > 0 ? $('#mapWrap').append($('<div/>', {
            id: 'yyExit',
            class: 'yy-hide yy-exit',
            text: '退出全屏',
            click: function () {
...
app.ui.deptTree = {
    ...
    init: function (cfg) {
        var treeElm = 'ycyaDeptTree',
            treeCfg = $.extend(true, this._cfg, cfg.cfg),
            _this = this;
        var _box = $("<div/>", {
            class: 'ycya-hide',
            id: 'ycyaDeptBox'
    ...
    init: function (cfg) {
        var boxId = 'mapSearch',
            ...
            _high = $('<span>', {
                class: 'cursor-point',
                id: 'seniorSearchBtn',
                html: '<i class="' + highCl + '"></i>'
            }),
            _low = $('<span>', {
                html: '<i class="' + lowCl + '"></i><strong><input placeholder="' + cfg["placeholder"] + '"></strong>'
            });
...
app.ui.master = {
    init: function (cfg) {
        ...
        var _box = $('<div/>', {
                class: 'master-btn cursor-point',
                click: function () {
                    $(this).find('ul,.up-triangle').toggle(500);
                }
            }),
            _boxi = $('<i/>', {
                class: "iconfont " + cfg.class
            }),
            _up = $('<div/>', {
                class: 'up-triangle master-list-triangle ycya-hide'
            });
        var _content = $('<ul/>', {
            class: 'master-list'
        });
...
app.ui.mapShow = {
    init: function (cfg) {
        ...
        var mapShowId = 'ycyaMapShow',
            ...
            _content = $('<div/>', {
                class: 'popup-panel show-box',
                html: '<div class="sub-title p-left-16">当前选择</div><div class="choosed-type"></div> <div class="choose-all p-left-16"><p class="to-choose-type-all">全部<i class="item-choosed"></i></p></div><ul class="to-choose-type clear p-left-16"></ul>'
            });
...
app.ui.statusList = {
    init: function (cfg) {
        var _this = this,
            ...
            _swrap = $('<div/>', {
                id: wrapId,
                class: 'ycya-hide layui-layer-wrap'
            }),
            _sbox = $('<div/>', {
                class: 'well-box clear'
            }),
            _slefttop = $('<div/>', {
                class: 'well-box-left-top'
            }),
            _sleftbottom = $('<div/>', {
                class: 'well-box-left-bottom',
                html: '<p>报警:</p><p class="alarm">' + cfg.left.alarmNum || 0 + '</p>'
            }),
            _sleft = $('<div/>', {
                class: 'well-box-left'
            }),
            _sright = $('<div/>', {
                class: 'well-box-right'
            }),
            _lefttitle = $('<p/>', {
                class: 'title',
                text: cfg.left.title
    ...
    createDivItem: function (obj) {
        return $('<div/>', {
            ...
            html: '<span>' + obj.name + '</span><strong class="rt">' + obj.num + '</strong><div class="layui-progress"><div class="layui-progress-bar ' + obj.bg + '" style="width:' + obj.percent + '%;"></div></div>'
        });
    },
    createList: function (cfg, itemList) {
        ...
        $.each(itemList, function (i, cur) {
            listArr.push($('<li/>', {
                html: '<span>' + cur[aliasName] + '：</span><span><i class="iconfont icon-blue  ' + cfg.icon + '"></i>' + cur[on] + '</span><span><i class="iconfont icon-red  ' + cfg.icon + '"></i>' + cur[alarm] + '</span><span><i class="iconfont icon-yellow  ' + cfg.icon + '"></i>' + cur[repair] + '</span><span><i class="iconfont icon-gray  ' + cfg.icon + '"></i>' + cur[off] + '</span>'
            })[0].outerHTML);
        });
    ...
    listRender: function (itemList) {
        ...
        $.each(itemList, function (i, cur) {
            listArr.push($('<li/>', {
                html: '<span>' + cur[_this.listCfg.aliasName] + '：</span><span><i class="iconfont icon-blue  ' + _this.listIcon + '"></i>' + cur[_this.listCfg.on] + '</span><span><i class="iconfont icon-red  ' + _this.listIcon + '"></i>' + cur[_this.listCfg.alarm] + '</span><span><i class="iconfont icon-yellow  ' + _this.listIcon + '"></i>' + cur[_this.listCfg.repair] + '</span><span><i class="iconfont icon-gray  ' + _this.listIcon + '"></i>' + cur[_this.listCfg.off] + '</span>'
            })[0].outerHTML);
        });
        this.ul.html(listArr.join(''));
    },
    set: function (jsonData) {
        $.each(this.leftElm, function (k, elm) {
            if (k == 'repairBar') {
                elm.css({
                    width: jsonData['repairNum'] / jsonData.allNum * 100 + '%'
                });
            }
            if (k == 'offBar') {
                elm.css({
                    width: jsonData['offNum'] / jsonData.allNum * 100 + '%'
                });
            }
...
app.ui.parkRank = {
    ...
    init: function (cfg) {
        ...
        var _parent = cfg.parent ? $(cfg.parent) : $('#bodyParkRank'),
            c = this._cfg,
            _this = this;
        $('.layui-card-header', _parent).prepend($('<span/>', {
            text: c.title || ''
        }));
        var thead = $('<thead/>'),
            theadtr = $('<tr/>'),
...
app.ui.page.parking = {
    init: function (cfg) {
        ...
        if (cfg.parkSort && cfg.parkSort.type == "pie") {
            var pieId = 'parksort';
            $('.layui-card-body', '#bodyParkSort').append($('<div/>', {
                height: 200,
                id: pieId
            }));
            $('.layui-card-header', '#bodyParkSort').append($('<span/>', {
                text: cfg.parkSort.title || ''
            }));
            $.extend(cfg.parkSort, {
                id: pieId
            });
        ...
        if (cfg.parkCurve && (cfg.parkCurve.type == "bar" || cfg.parkCurve.type == "line")) {
            ...
            $('.layui-card-body', '#bodyParkCurve').append($('<div/>', {
                height: 300,
                id: curseId
            }));
            $('.layui-card-header', '#bodyParkCurve').append($('<span/>', {
                text: cfg.parkCurve.title || ''
            }));
            $.extend(cfg.parkCurve, {
                id: curseId
            });
    ...
    getParkRank: function () {
        return this.parkRank;
    },
    getParkSort: function () {
        return this.parkSort;
    },
    getParkCurve: function () {
        return this.parkCurve;
    }
};
$(function () {
